...
import {
    ...
    CourseDocument,
    courseSchema,
} from '@app/model/database/course';
import {
    getConnectionToken,
    getModelToken,
    MongooseModule,
...
describe('CourseServiceEndToEnd', () => {
    ...
    beforeEach(async () => {
        ...
        service = module.get<CourseService>(CourseService);
        courseModel = module.get<Model<CourseDocument>>(
    ...
    it('getCourse() return course with the specified subject code', async () => {
        const course = getFakeCourse();
        await courseModel.create(course);
        expect(await service.getCourse(course.subjectCode)).toEqual(
    ...
    it('getCourseTeacher() should fail if course does not exist', async () => {
        const course = getFakeCourse();
        await expect(
            service.getCourseTeacher(course.teacher),
        ).rejects.toBeTruthy();
    });
    ...
    it('deleteCourse() should fail if the course does not exist', async () => {
        ...
        const course = getFakeCourse();
        await expect(
            service.deleteCourse(course.subjectCode),
        ).rejects.toBeTruthy();
    });
    ...
    it('addCourse() should fail if mongo query failed', async () => {
        jest.spyOn(courseModel, 'create').mockImplementation(async () =>
            Promise.reject(''),
        );
        const course = getFakeCourse();
        await expect(
            service.addCourse({ ...course, subjectCode: 'INF', credits: 5 }),
        ).rejects.toBeTruthy();
    });
    ...
    it('addCourse() should fail if the course is not a valid', async () => {
        const course = getFakeCourse();
        await expect(
            service.addCourse({ ...course, subjectCode: 'IND', credits: 5 }),
        ).rejects.toBeTruthy();
        await expect(
            service.addCourse({ ...course, subjectCode: 'INF', credits: 90 }),
        ).rejects.toBeTruthy();
        await expect(
            service.addCourse({ ...course, subjectCode: 'IND', credits: 90 }),
        ).rejects.toBeTruthy();
    });
});
...
const getRandomString = (): string =>
    (Math.random() + 1).toString(BASE_36).substring(2);
