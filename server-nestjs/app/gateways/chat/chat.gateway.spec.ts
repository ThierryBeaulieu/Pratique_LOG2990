...
describe('ChatGateway', () => {
    ...
    it('validate() message should take account word length', () => {
        ...
        for (const { word, isValid } of testCases) {
            ...
            expect(
                socket.emit.calledWith(ChatEvents.WordValidated, isValid),
            ).toBeTruthy();
        }
    });
    ...
    it('broadcastAll() should send a mass message to the server', () => {
        ...
        expect(
            server.emit.calledWith(ChatEvents.MassMessage, match.any),
        ).toBeTruthy();
    });
    ...
    it('afterInit() should emit time after 1s', () => {
        ...
        expect(
            server.emit.calledWith(ChatEvents.Clock, match.any),
        ).toBeTruthy();
    });
    ...
    it('hello message should be sent on connection', () => {
        ...
        expect(
            socket.emit.calledWith(ChatEvents.Hello, match.any),
        ).toBeTruthy();
    });
